...
async function callLLM(systemPrompt: string, userPrompt: string, isJson: boolean = false): Promise<string> {
    ...
    try {
        ...
        const tagsRes = await fetch(`${ollamaBaseUrl}/api/tags`, { signal: getTimeoutSignal(2000) });
        if (tagsRes.ok) {
            const data = await tagsRes.json() as any;
            const models = data.models || [];
            if (models.length > 0) {
                const modelName = process.env.OLLAMA_MODEL || models[0].name;
                console.log(`Ollama active! Using model: ${modelName}`);
                ...
                if (chatRes.ok) {
                    ...
                    console.error("Ollama API returned status:", chatRes.status, await chatRes.text());
                }
            } else {
                console.log("Ollama running but no local models found. Use 'ollama pull <model>' first.");
            }
        }
    } catch (err) {
...
`;
        ...
        if (!reply) {
            ...
            reply = fallbackQuestions[Math.min(qIndex, fallbackQuestions.length - 1)]!;
        }
...
`;
        ...
        if (reply) {
            try {
                ...
                console.error("Failed to parse LLM JSON response during evaluation:", reply, err);
            }
        }

        if (!result) {
            throw new Error("No active AI key or local model succeeded in evaluation.");
        }
        ...
        console.log("Evaluation complete for interview:", interviewId, "Score:", result.score);
